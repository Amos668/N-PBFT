...
func genRsaKeys() {
	if !isExist("./Keys") {
		...
		for i := 0; i <= 4; i++ {
			if !isExist("./Keys/P" + strconv.Itoa(i)) {
				err := os.Mkdir("./Keys/P"+strconv.Itoa(i), 0777)
				if err != nil {
					log.Panic()
				}
			}
			priv, pub := getKeyPair()
			privFileName := "Keys/P" + strconv.Itoa(i) + "/P" + strconv.Itoa(i) + "_RSA_PIV"
			...
			pubFileName := "Keys/P" + strconv.Itoa(i) + "/P" + strconv.Itoa(i) + "_RSA_PUB"
